...
LineaMicra::LineaMicra(HomeAssistantClient* client)
    : haClient(client),
      ...
      _preBrewTime(0.0f),
      _preBrewWait(0.0f) {
  client->subscribeToEvent(LINEA_MICRA_ENTITY_ID, [this](const HAEntity& entity) {
  ...
  client->subscribeToEvent(LINEA_MICRA_BOILER_TEMP_ENTITY_ID, [this](const HAEntity& entity) {
  ...
  client->subscribeToEvent(LINEA_MICRA_PREBREW_TIME_ENTITY_ID, [this](const HAEntity& entity) {
    _preBrewTime = entity.state.toFloat();
    Serial.println("Linea Micra pre-brew time updated: " + String(_preBrewTime));
  });

  client->subscribeToEvent(LINEA_MICRA_PREBREW_WAIT_ENTITY_ID, [this](const HAEntity& entity) {
    _preBrewWait = entity.state.toFloat();
    Serial.println("Linea Micra pre-brew wait time updated: " + String(_preBrewWait));
  });

  client->subscribeToEvent(LINEA_MICRA_PREBREW_MODE_ENTITY_ID, [this](const HAEntity& entity) {
